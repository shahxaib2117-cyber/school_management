...
const Students = () => {
  ...
  // console.log("🚀 ~ Students ~ logedInTeacher:", logedInTeacher)
  const handleLogout = () => {
    logout()
  ...
  const check = studentsOfCurrentTeacher?.length === 0 && students.length === 0;
      ...
      <div className={`w-19/20 flex relative `}>
        <div className={`w-19/20 flex-1 flex flex-col gap-8
         ${check ? `items-end  pr-10 pb-10 ` : `items-start pl-5 `}`} >
          ...
          {/* list-child */}
          <div className={` flex-1 flex ${check ? `w-87/100 justify-end items-end` : `w-10/10`} bg-[#e7eaeee5] 
        rounded-[5px] `} >
            {/* contant */}
            {check ? <div className="flex gap-25 ">
              {/* child-1 */}
